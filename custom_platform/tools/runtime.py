"""Stable runtime selection helpers for custom_platform tools."""
...
def render_runtime_markdown(info: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- profile: `{commands.get('profile', '')}`",
    ...
    if uv_no_sync:
        lines.extend(
            [
                ...
                f"- profile: `{uv_no_sync.get('profile', '')}`",
