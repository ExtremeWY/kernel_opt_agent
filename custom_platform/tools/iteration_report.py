...
def render_final_summary(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- active routes: {len((strategy_scope.get('routes') or {}).keys())}",
        f"- design boundary active: {(strategy_scope.get('design_boundary') or {}).get('active')}",
        f"- design boundary reason: {(strategy_scope.get('design_boundary') or {}).get('reason', '')}",
        "",
        "## Iterations",
        "",
