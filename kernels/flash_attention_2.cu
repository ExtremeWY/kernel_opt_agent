...
__global__ void flash_attention_2_forward_kernel(
  ...
  auto* shared = reinterpret_cast<__nv_bfloat16*>(smem_raw);
  __nv_bfloat16* k_tile = shared;
  __nv_bfloat16* v_tile = shared + BLOCK_N * kHeadDim;
  ...
  const int64_t bh_offset =
      (static_cast<int64_t>(batch_idx) * num_heads + head_idx) * seq_len * kHeadDim;

  if (query_idx >= seq_len) {
    return;
  }

  float q_frag[kElementsPerLane];
  float acc[kElementsPerLane];
#pragma unroll
  for (int i = 0; i < kElementsPerLane; ++i) {
    const int dim = lane_id * kElementsPerLane + i;
    q_frag[i] = __bfloat162float(q[bh_offset + static_cast<int64_t>(query_idx) * kHeadDim + dim]);
    acc[i] = 0.0f;
  }
  ...
  for (int start_n = 0; start_n < seq_len; start_n += BLOCK_N) {
    ...
    for (int linear = threadIdx.x; linear < BLOCK_N * kHeadDim; linear += blockDim.x) {
      const int row = linear / kHeadDim;
      const int col = linear % kHeadDim;
      const int key_idx = start_n + row;
      const int64_t global_idx = bh_offset + static_cast<int64_t>(key_idx) * kHeadDim + col;
      if (row < tile_count) {
        k_tile[linear] = k[global_idx];
        v_tile[linear] = v[global_idx];
      } else {
        k_tile[linear] = __float2bfloat16(0.0f);
        v_tile[linear] = __float2bfloat16(0.0f);
      }
    }
    __syncthreads();
...
#pragma unroll
      for (int i = 0; i < kElementsPerLane; ++i) {
        const int dim = lane_id * kElementsPerLane + i;
        score_partial += q_frag[i] * __bfloat162float(k_tile[local_n * kHeadDim + dim]);
      }
...
#pragma unroll
      for (int i = 0; i < kElementsPerLane; ++i) {
        const int dim = lane_id * kElementsPerLane + i;
        const float v_val = __bfloat162float(v_tile[local_n * kHeadDim + dim]);
        acc[i] = acc[i] * alpha + p * v_val;
      }
      m_i = new_m;
      l_i = new_l;
    }
...
#pragma unroll
  for (int i = 0; i < kElementsPerLane; ++i) {
    const int dim = lane_id * kElementsPerLane + i;
    o[bh_offset + static_cast<int64_t>(query_idx) * kHeadDim + dim] =
...
torch::Tensor flash_attention_2_forward(
  ...
  const size_t shared_mem_bytes = 2 * block_n * kHeadDim * sizeof(__nv_bfloat16);
